...
alongside the features, in [make/make.md](make/make.md). The **`-Z` setting**,
through which the features in the `Flag` column are enabled by name, is likewise
not a patch: it is the stream base branch `feature/<stream>/zflags`, on which
every patch sits, documented in [zflags/zflags.md](zflags/zflags.md). A feature
whose `Flag` is "always on" runs unconditionally. [boundscap](boundscap/boundscap.md)
