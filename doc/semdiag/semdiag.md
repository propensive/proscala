...
Adds `-Zsemantic-diagnostics`, which replaces the console error output with a
...
Enabled by `-Zsemantic-diagnostics` ([zflags](../zflags/zflags.md)); without it, upstream's code runs at every site this patch touches.
