...
Scala.js 1.17+ has an experimental WasmBackend that compiles Scala.js IR directly to WebAssembly (rather than JavaScript), but upstream it still targets the JS embedding. The **scala-wasm** project extends that backend to emit standalone WASM *components* — which requires the compiler to know which Scala definitions correspond to WIT imports, exports and data types. `feature/<stream>/wasm` vendors that support onto the Proscala compiler as a single patch on `upstream/<stream>`: the backend/codegen changes and the `scala.scalajs.wit` user-facing library in `library-js`, plus a fix for a `ScopedVar` crash when generating WIT member defs at class scope. The build-side pieces are not on the branch — they live on `main`, like the rest of the build: the WIT-extended Scala.js IR (v1.22.0-wasm.4) is vendored at `mk/scalajs-ir/wasm/` and dropped into `compiler/src-scalajs-ir` by `bin/proscala-overlay`, and the switch of the sjs stdlib to the `io.github.scala-wasm` runtime is made by the Makefile, both applied automatically to any tree carrying the WIT sources.
