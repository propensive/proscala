...
class Jsr45Test extends DottyBytecodeTest:
  ...
  @Test def inlineAcrossFiles(): Unit =
    ...
    checkBCode(List(util, main)) { dir =>
      val utilClass = loadClassNode(dir.lookupName("Util$.class", directory = false).nn.input, skipDebugInfo = false)
      assertNull("unit without foreign inlining should carry no SMAP", utilClass.sourceDebug)

      val mainClass = loadClassNode(dir.lookupName("Main.class", directory = false).nn.input, skipDebugInfo = false)
